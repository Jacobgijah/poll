...
pollForm.addEventListener('submit', async (e) => {
  ...
  // Ensure checkedValue is not null
  if (!checkedValue) {
    console.error("No option selected or formData is null.");
    return;
  }
  
  console.log("Checked Value:", checkedValue);
  if (!checkedValue) {
      console.error("No option selected. Please select an option before voting.");
  }
...
function displayResults() {
  if (resultsDiv === null) {
    console.error("The element with ID 'results' is not found.");
    return;
  }
